...
// checkPolicy enforces c.Policy.Requires against the fields actually
...
func checkPolicy(filter *Condition, c *Config) *PolicyViolationError {
...
func policyMessage(r FieldRequirement) string {
	if r.Message != "" {
		return r.Message
	}
...
// all", not where.
func filterFieldOperators(cond *Condition) map[string]map[Operator]bool {
	used := make(map[string]map[Operator]bool)
	var walk func(*Condition)
	walk = func(c *Condition) {
		if c == nil {
			return
		}
		if c.Type == CondComparison {
			if used[c.Field] == nil {
				used[c.Field] = make(map[Operator]bool)
			}
...
func anyOperatorIn(used map[Operator]bool, ops []string) bool {
	for _, op := range ops {
		if used[Operator(op)] {
			return true
		}
...
func anyFieldIn(used map[string]map[Operator]bool, fields []string) bool {
