...
// method.
func (p *OpenAIProvider) SetObserver(o observe.Observer) { p.Observe = o }
...
func (p *OpenAIProvider) roundTrip(ctx context.Context, system, user string, retry int) (out string, err error) {
	...
	defer func() {
		...
		if err != nil {
			ev.Outcome = observe.OutcomeTransport // no usable text came back, whatever the cause
			ev.Err = err
	...
	if p.BaseURL == "" {
		return "", noEndpointError(config.ModelConfig{Provider: p.ProviderID})
	}
	...
	ev.HiddenTokens = observe.HiddenTokens(cr.Usage.TotalTokens, cr.Usage.PromptTokens, cr.Usage.CompletionTokens)
	if len(cr.Choices) > 0 {
	...
	if len(cr.Choices) == 0 {
		return "", p.fail(failure.KindBadResponse, resp.StatusCode, "response contained no choices", nil)
	}
...
func (p *OpenAIProvider) fail(kind failure.ProviderErrorKind, status int, detail string, cause error) *ProviderError {
