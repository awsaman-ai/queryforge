...
type AnthropicProvider struct {
	...
	Observe    observe.Observer // optional; receives one EventModelCall per round trip
...
func (p *AnthropicProvider) SetObserver(o observe.Observer) { p.Observe = o }
...
func (p *AnthropicProvider) roundTrip(ctx context.Context, system, user string, retry int) (out string, err error) {
	...
	start := time.Now()
	ev := observe.Event{Kind: observe.EventModelCall, Provider: p.ProviderID, Model: p.Model, Outcome: observe.OutcomeOK, Retry: retry}
	defer func() {
		ev.Latency = time.Since(start)
		if err != nil {
			ev.Outcome = observe.OutcomeTransport
			ev.Err = err
			if pe, ok := asProviderError(err); ok {
				ev.ErrorKind = pe.Kind
			}
		}
		observe.Emit(p.Observe, ctx, ev)
	}()

	if p.APIKey == "" {
		return "", p.fail(failure.KindAuth, 0, "no API key (set the env var named in the config's apiKeyEnv)", nil)
	}
	...
	if err != nil {
		return "", p.fail(failure.KindInvalidRequest, 0, "marshal request: "+err.Error(), err)
	}
	...
	if err != nil {
		return "", p.fail(failure.KindInvalidRequest, 0, "build request: "+err.Error(), err)
	}
	...
	if err := json.Unmarshal(data, &ar); err != nil {
		return "", p.fail(failure.KindBadResponse, resp.StatusCode, "decode response: "+err.Error(), err)
	}
	...
	if ar.Error != nil && ar.Error.Message != "" {
		kind := failure.KindBadResponse
		if looksLikeQuota(ar.Error.Message) {
			kind = failure.KindQuota
		}
		return "", p.fail(kind, resp.StatusCode, "model error: "+ar.Error.Message, nil)
	}
	if ar.StopReason == "refusal" {
		return "", p.fail(failure.KindBadResponse, resp.StatusCode, "request was refused by the model's safety classifier", nil)
	}
	...
	if ar.StopReason == "max_tokens" {
		return "", p.fail(failure.KindBadResponse, resp.StatusCode, fmt.Sprintf(
	...
	out = b.String()
	if out == "" {
		return "", p.fail(failure.KindBadResponse, resp.StatusCode, "response contained no text content", nil)
	}
	return out, nil
}
...
func (p *AnthropicProvider) fail(kind failure.ProviderErrorKind, status int, detail string, cause error) *ProviderError {
...
func ProviderFor(m config.ModelConfig) ModelProvider {
