...
func (f *FallbackProvider) SetObserver(o observe.Observer) {
	for _, e := range f.entries {
...
// Namer is implemented by providers that can report which model answered
...
func ProvidersFrom(c *config.Config) ModelProvider {
	var entries []fallbackEntry
	add := func(m config.ModelConfig) {
		if config.IsEmptyModel(m) {
