// Package validate is the deterministic guarantee: it decides whether a Query
// AST can be expressed in a given config's vocabulary, and says exactly which
// rule it broke when it cannot.
//
// regexguard.go is a second file in the same package rather than a package of its
// own because its only caller is the regex-policy check here.
package validate
...
import (
	...
	"github.com/awsaman-ai/queryforge/internal/valuetype"
)
...
func Validate(q *ast.Query, c *config.Config) error {
	...
	if !supportedASTVersion(q.Version) {
		errs = append(errs, &ValidationError{
			...
			Path:    "version",
			Message: fmt.Sprintf("unsupported AST version %q (this library understands %q)", q.Version, ast.ASTVersion),
		})
	}
	...
	if perr := policy.Check(q.Filter, c); perr != nil {
		return perr
	}
...
func (w *walk) suggest(name string) []string {
	if max := config.EffectiveMaxSuggestCalls(w.c.Policy); max > 0 && w.suggests >= max {
		return nil
	}
	w.suggests++
	return suggestFields(w.c, name)
}

// condition recursively validates one node of the filter tree. depth is the
// node's own depth, counted from 1 at the root.
func (w *walk) condition(path string, cond *ast.Condition, depth int) ValidationErrors {
	...
	w.nodes++
	if max := config.EffectiveMaxFilterNodes(w.c.Policy); max > 0 && w.nodes > max {
		if w.overrun {
...
func (w *walk) logical(path string, cond *ast.Condition, depth int) ValidationErrors {
	var errs ValidationErrors

	switch cond.Op { // the connective must be one of the three known ones
	case ast.OpAND, ast.OpOR, ast.OpNOT:
	default:
	...
	if cond.Op == ast.OpNOT && len(cond.Children) != 1 { // NOT negates exactly one subtree
		errs = append(errs, &ValidationError{Code: CodeInvalidArity, Path: path,
...
func (w *walk) comparison(path string, cond *ast.Condition) ValidationErrors {
	c := w.c
	var errs ValidationErrors
	...
	if !config.IsKnownOperator(cond.Operator) {
	...
	if !f.AllowsOperator(cond.Operator) {
		return ValidationErrors{{Code: CodeOperatorNotAllowed, Path: path, Field: cond.Field,
			Message: fmt.Sprintf("operator %q not allowed on field %q (allowed: %s)",
				cond.Operator, cond.Field, ast.JoinOperators(f.EffectiveOperators()))}}
	}
	// Text-search operators on a string/enum field require the searchable flag.
	if (f.Type == config.FieldString || f.Type == config.FieldEnum) && config.IsTextSearchOperator(cond.Operator) && !f.EffectiveSearchable() {
	...
	if cond.Operator == ast.OpRegex {
		errs = append(errs, checkRegexPolicy(c, path, cond)...)
	}

	// 4) Value shape. Null operators take no value; everything else requires one.
	if ast.IsNullOperator(cond.Operator) {
		if cond.Value != nil {
	...
	elemType := valuetype.ScalarTypeOf(f)
...
func ValidateArrayValue(c *config.Config, path string, f *config.Field, elemType config.FieldType, cond *ast.Condition) ValidationErrors {
	var errs ValidationErrors
	...
	if cond.Operator == ast.OpBetween && len(elems) != 2 { // BETWEEN is a 2-tuple
	...
	if cond.Operator != ast.OpBetween && len(elems) == 0 { // in/contains* need at least one
		errs = append(errs, &ValidationError{Code: CodeInvalidArity, Path: path, Field: cond.Field,
	...
	if max := config.EffectiveMaxListLength(c.Policy); max > 0 && len(elems) > max {
	...
	for i, e := range elems { // type-check each element against the element type
		...
		if elemType == config.FieldEnum { // enum elements must be in-domain
			if s, ok := e.(string); ok && !config.HasEnumValue(*f, s) {
		...
		if elemType == config.FieldNumber { // numeric elements honor min/max validators
			if n, ok := e.(float64); ok {
				errs = append(errs, checkNumericBounds(f, path, n)...)
			}
		}
		if s, ok := e.(string); ok { // one long element is as costly as one long scalar
			errs = append(errs, checkValueLength(c, fmt.Sprintf("%s.value[%d]", path, i), cond.Field, s)...)
		}
	}
	return errs
}
...
func checkValueLength(c *config.Config, path, field, s string) ValidationErrors {
	max := config.EffectiveMaxValueLength(c.Policy)
	if max <= 0 || len(s) <= max {
		return nil
	}
...
func ValidateScalarValue(c *config.Config, path string, f *config.Field, elemType config.FieldType, v *ast.Value) ValidationErrors {
	var errs ValidationErrors
	...
	if v != nil && v.Kind == ast.KindRelativeDate {
	...
	if elemType == config.FieldEnum { // enum scalar must be in-domain
		if s, ok := v.AsString(); ok && !config.HasEnumValue(*f, s) {
	...
	if elemType == config.FieldNumber { // numeric scalar honors min/max validators
		if n, ok := v.AsFloat(); ok {
			errs = append(errs, checkNumericBounds(f, path, n)...)
		}
	}
	if s, ok := v.AsString(); ok { // bound the literal's size
		errs = append(errs, checkValueLength(c, path, f.Name, s)...)
	}
	return errs
}
...
// nanoseconds — see datetime.MaxRelativeAmount).
func validateRelativeDate(path string, f *config.Field, elemType config.FieldType, v *ast.Value) ValidationErrors {
	var errs ValidationErrors
	if elemType != config.FieldDate {
		return ValidationErrors{{Code: CodeKindMismatch, Path: path, Field: f.Name,
			Message: fmt.Sprintf("value %s is not compatible with field type %q", describePayload(v), elemType)}}
	}
	if !datetime.IsRelativeUnit(v.Unit) {
		errs = append(errs, &ValidationError{Code: CodeInvalidRelDate, Path: path, Field: f.Name,
			Message: fmt.Sprintf("relative date unit %q is not one of: %s", v.Unit, strings.Join(datetime.RelativeUnits, ", "))})
	}
	if v.Amount > datetime.MaxRelativeAmount || v.Amount < -datetime.MaxRelativeAmount {
		errs = append(errs, &ValidationError{Code: CodeInvalidRelDate, Path: path, Field: f.Name,
			Message: fmt.Sprintf("relative date amount %d is out of range (max magnitude %d)", v.Amount, datetime.MaxRelativeAmount)})
	}
	return errs
}
...
func scalarKindOK(t config.FieldType, v *ast.Value) bool {
	switch t {
	case config.FieldString, config.FieldEnum: // both are carried as text; enum adds a domain check
		_, ok := v.V.(string)
		return ok
	...
	case config.FieldDate:
		...
		s, ok := v.V.(string)
		return ok && datetime.IsDateString(s)
	default:
...
func describePayload(v *ast.Value) string {
	if v.Kind == ast.KindRelativeDate {
		return "relative date"
	}
	switch p := v.V.(type) {
	case string:
		if datetime.IsDateString(p) {
...
func elementKindOK(t config.FieldType, e any) bool {
	switch t {
	case config.FieldString, config.FieldEnum:
		_, ok := e.(string)
		return ok
	case config.FieldDate:
...
func checkNumericBounds(f *config.Field, path string, n float64) ValidationErrors {
	if f.Validators == nil {
		return nil
	}
	var errs ValidationErrors
...
func regexDenied(c *config.Config, field string) bool {
...
func regexAllowed(c *config.Config, field string) bool {
...
func checkRegexPolicy(c *config.Config, path string, cond *ast.Condition) ValidationErrors {
	var errs ValidationErrors
	if regexDenied(c, cond.Field) {
	...
	} else if !regexAllowed(c, cond.Field) {
	...
	if cond.Value != nil {
		if s, ok := cond.Value.AsString(); ok {
			if max := config.EffectiveMaxRegexLength(c.Policy); max > 0 && len(s) > max {
...
func suggestFields(c *config.Config, name string) []string {
	type cand struct {
		name string
		dist int
	}
