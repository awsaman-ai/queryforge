...
//
// A package-level function rather than a method, like the four below it: Config,
// Field and ValueCase are all re-exported verbatim as queryforge types, and a new
// exported method on any of them would widen the library's public API as a side
// effect of an internal reorganisation.
func ValueCaseFor(c *Config, fieldName string) ValueCase {
...
func CaseInsensitiveFor(c *Config, fieldName string) bool {
	f, ok := c.fieldByName[fieldName]
	if !ok {
		return false
	}
...
// ApplyCase folds a single string according to the rule. CaseAsIs returns the
// input unchanged, so callers can apply it unconditionally.
func ApplyCase(vc ValueCase, s string) string {
	switch vc {
	case CaseLower:
...
// ApplyCaseAny folds v when it is a string and returns everything else
// untouched. Array elements arrive as `any` (numbers, bools, already-parsed
// dates), and a case rule must never reshape a non-string element.
func ApplyCaseAny(vc ValueCase, v any) any {
	if s, ok := v.(string); ok {
		return ApplyCase(vc, s)
	}
	return v
}

// ApplyCaseElems folds every string element of an array value.
//
...
// allocates nothing.
func ApplyCaseElems(vc ValueCase, elems []any) []any {
	if vc == CaseAsIs {
		return elems
	}
	out := make([]any, len(elems))
	for i, e := range elems {
		out[i] = ApplyCaseAny(vc, e)
	}
	return out
}
