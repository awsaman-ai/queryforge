...
func (c *Config) validateRoutingCondition(where string, cond RoutingCondition, routingField func(string) error) error {
	...
	case ast.OpEquals, ast.OpNotEquals, ast.OpGt, ast.OpGte, ast.OpLt, ast.OpLte:
	default:
	...
	if cond.Operator != ast.OpEquals && cond.Operator != ast.OpNotEquals && f.Type != FieldNumber && f.Type != FieldDate {
