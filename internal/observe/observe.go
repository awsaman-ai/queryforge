// Package observe is the observability seam: the Event struct the library
// reports facts through, the Observer callback that receives them, and the slog
// adapter (slog.go) almost every caller was going to write by hand.
//
// It imports internal/failure — for Classify, which the slog adapter needs to put
// an error_code on a log line, and for ProviderErrorKind, which Event carries —
// and nothing else in the library. Nothing here logs, formats a message, or picks
// a severity on its own initiative.
package observe
...
type Event struct {
	...
	ErrorKind failure.ProviderErrorKind
...
//
// A package-level function rather than the method it used to be: Observer is
// re-exported verbatim as queryforge.Observer, and exporting a method on it to
// reach it from another package would add that method to the library's public
// API — on the one type whose whole contract is "it is just a func".
func Emit(o Observer, ctx context.Context, e Event) {
	if o == nil {
		return
	}
	o(ctx, e)
