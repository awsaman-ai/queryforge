...
func (g MongoGenerator) Generate(q *ast.Query, c *config.Config, opts GenOptions) (*Result, error) {
	now := opts.now()
	mq := &MongoQuery{
...
func mongoCollection(q *ast.Query, c *config.Config) string {
...
func mongoCondition(c *config.Config, cond *ast.Condition, now time.Time) (map[string]any, error) {
	switch cond.Type {
...
func mongoLogical(c *config.Config, cond *ast.Condition, now time.Time) (map[string]any, error) {
...
func mongoExistenceGuard(c *config.Config, child *ast.Condition) (map[string]any, bool) {
...
func mongoComparison(c *config.Config, cond *ast.Condition, now time.Time) (map[string]any, error) {
...
func mongoPredicate(c *config.Config, cond *ast.Condition, field string, now time.Time) (map[string]any, error) {
	...
	case ast.OpEquals:
		if ci {
			...
			s, err := mongoString(cond, ast.OpEquals)
			if err != nil {
				return nil, err
			}
	...
	case ast.OpNotEquals:
		if ci {
			s, err := mongoString(cond, ast.OpNotEquals)
			if err != nil {
				return nil, err
			}
	...
	case ast.OpBefore:
		return mongoScalarExpr(field, "$lte", scalar, false)
	case ast.OpBetween:
		elems, err := sliceValue("mongo", ast.OpBetween, cond.Value, 2)
		if err != nil {
			return nil, err
		}
	...
	case ast.OpIn:
		elems, err := sliceValue("mongo", ast.OpIn, cond.Value, 0)
		if err != nil {
			return nil, err
		}
		if ci {
	...
	case ast.OpNotIn:
		elems, err := sliceValue("mongo", ast.OpNotIn, cond.Value, 0)
		if err != nil {
			return nil, err
		}
		if ci {
	...
	case ast.OpContainsAll:
		elems, err := sliceValue("mongo", ast.OpContainsAll, cond.Value, 0)
		if err != nil {
			return nil, err
		}
		return expr(field, "$all", mongoElems(elems, elemType, vc)), nil
	case ast.OpContainsAny:
		elems, err := sliceValue("mongo", ast.OpContainsAny, cond.Value, 0)
		if err != nil {
			return nil, err
		}
		return expr(field, "$in", mongoElems(elems, elemType, vc)), nil
	case ast.OpContains:
		...
		s, err := mongoString(cond, ast.OpContains)
		if err != nil {
			return nil, err
		}
		return regexExpr(field, regexp.QuoteMeta(config.ApplyCase(vc, s)), "i"), nil
	case ast.OpStartsWith:
		s, err := mongoString(cond, ast.OpStartsWith)
		if err != nil {
			return nil, err
		}
		return regexExpr(field, "^"+regexp.QuoteMeta(config.ApplyCase(vc, s)), regexOptions(ci)), nil
	case ast.OpEndsWith:
		s, err := mongoString(cond, ast.OpEndsWith)
		if err != nil {
			return nil, err
		}
		return regexExpr(field, regexp.QuoteMeta(config.ApplyCase(vc, s))+"$", regexOptions(ci)), nil
	case ast.OpRegex:
		s, err := mongoString(cond, ast.OpRegex)
		if err != nil {
			return nil, err
		}
...
func mongoString(cond *ast.Condition, op ast.Operator) (string, error) {
	if cond.Value == nil {
...
func mongoScalar(v *ast.Value, declared config.FieldType, now time.Time, vc config.ValueCase) (any, error) {
	if v == nil {
		return nil, fmt.Errorf("mongo: predicate requires a value")
	}
	if v.Kind == ast.KindRelativeDate { // no payload: unit/amount resolve against the clock
	...
	case config.FieldDate:
		s, _ := v.AsString()
		return datetime.ParseDate(s), nil
...
func mongoElems(elems []any, elemType config.FieldType, vc config.ValueCase) []any {
	out := make([]any, len(elems))
	for i, e := range elems {
		if elemType == config.FieldDate {
			if s, ok := e.(string); ok {
				out[i] = datetime.ParseDate(s)
				continue
			}
		}
