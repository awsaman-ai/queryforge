...
func (ESGenerator) Generate(q *ast.Query, c *config.Config, opts GenOptions) (*Result, error) {
...
func (OpenSearchGenerator) Generate(q *ast.Query, c *config.Config, opts GenOptions) (*Result, error) {
...
type ESQuery struct {
	...
	SourceType config.SourceType `json:"sourceType"`
...
func generateES(backend string, q *ast.Query, c *config.Config, opts GenOptions) (*Result, error) {
	now := opts.now()
...
func esExactPath(c *config.Config, backend, field string) string {
...
// representation.
func esTextPath(c *config.Config, backend, field string) string {
	return c.PhysicalName(field, backend)
}

// esCondition dispatches on node type, returning one query clause.
func esCondition(backend string, c *config.Config, cond *ast.Condition, now time.Time) (map[string]any, error) {
	switch cond.Type {
...
func esLogical(backend string, c *config.Config, cond *ast.Condition, now time.Time) (map[string]any, error) {
...
func esExistenceGuard(backend string, c *config.Config, child *ast.Condition) (map[string]any, bool) {
	if child == nil || child.Type != ast.CondComparison || ast.IsNullOperator(child.Operator) {
		return nil, false
	}
...
func esComparisonWrapped(backend string, c *config.Config, cond *ast.Condition, now time.Time) (map[string]any, error) {
...
func esComparison(backend string, c *config.Config, cond *ast.Condition, now time.Time) (map[string]any, error) {
	elemType := valuetype.CondScalarType(c, cond)
	vc := config.ValueCaseFor(c, cond.Field)
	...
	ci := elemType == config.FieldString && config.CaseInsensitiveFor(c, cond.Field)
	...
	case ast.OpAfter: // inclusive, per design doc — matches every other generator
		v, err := scalar()
		if err != nil {
			return nil, err
		}
	...
	case ast.OpBetween:
		elems, err := sliceValue(backend, ast.OpBetween, cond.Value, 2)
		if err != nil {
			return nil, err
		}
	...
	case ast.OpIn:
		elems, err := sliceValue(backend, ast.OpIn, cond.Value, 0)
		if err != nil {
			return nil, err
		}
		return esTerms(exact, esElems(elems, elemType, vc), ci), nil
	case ast.OpNotIn:
		elems, err := sliceValue(backend, ast.OpNotIn, cond.Value, 0)
		if err != nil {
			return nil, err
		}
	...
	case ast.OpContainsAll:
		elems, err := sliceValue(backend, ast.OpContainsAll, cond.Value, 0)
		if err != nil {
			return nil, err
		}
		conv := esElems(elems, elemType, vc)
	...
	case ast.OpContainsAny:
		elems, err := sliceValue(backend, ast.OpContainsAny, cond.Value, 0)
		if err != nil {
			return nil, err
		}
		return esTerms(exact, esElems(elems, elemType, vc), ci), nil
	case ast.OpContains:
		...
		s, err := esString(cond, ast.OpContains)
		if err != nil {
			return nil, err
		}
		return map[string]any{"match": map[string]any{text: config.ApplyCase(vc, s)}}, nil
	case ast.OpStartsWith:
		s, err := esString(cond, ast.OpStartsWith)
		if err != nil {
			return nil, err
		}
		return esPrefix(exact, config.ApplyCase(vc, s), ci), nil
	case ast.OpEndsWith:
		...
		s, err := esString(cond, ast.OpEndsWith)
		if err != nil {
			return nil, err
		}
		return esWildcard(exact, "*"+esEscapeWildcard(config.ApplyCase(vc, s)), ci), nil
	case ast.OpRegex:
		s, err := esString(cond, ast.OpRegex)
		if err != nil {
			return nil, err
		}
...
func esRangeOp(op ast.Operator) string {
	switch op {
	case ast.OpGt:
		return "gt"
	case ast.OpGte:
...
func esString(cond *ast.Condition, op ast.Operator) (string, error) {
	if cond.Value == nil {
...
func esValue(v *ast.Value, declared config.FieldType, now time.Time, vc config.ValueCase) (any, error) {
	if v == nil {
...
func esElems(elems []any, elemType config.FieldType, vc config.ValueCase) []any {
	...
	for i, e := range elems {
		...
		out[i] = config.ApplyCaseAny(vc, e)
	}
	return out
}
