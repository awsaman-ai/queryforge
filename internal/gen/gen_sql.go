...
type sqlDialect struct {
	...
	quoter      identifier.IdentQuoter // identifier delimiters
...
var postgresDialect = sqlDialect{
	backend:     "sql",
	quoter:      identifier.IdentQuoter{Open: `"`, Close: `"`},
...
func (SQLGenerator) Generate(q *ast.Query, c *config.Config, opts GenOptions) (*Result, error) {
...
func (MySQLGenerator) Generate(q *ast.Query, c *config.Config, opts GenOptions) (*Result, error) {
	return generateSQL(mysqlDialect, q, c, opts)
}

// generateSQL is the dialect-independent compiler.
func generateSQL(d sqlDialect, q *ast.Query, c *config.Config, opts GenOptions) (*Result, error) {
...
func sqlTable(q *ast.Query, c *config.Config, backend string) string {
...
func sqlPhysicalName(c *config.Config, field, backend string) string {
...
func (b *sqlBuilder) ident(name string) (string, error) {
	...
	return identifier.QuoteIdentPath(name, b.dialect.quoter), nil
}

// condition dispatches on node type.
func (b *sqlBuilder) condition(c *config.Config, cond *ast.Condition) (string, error) {
	switch cond.Type {
...
func (b *sqlBuilder) logical(c *config.Config, cond *ast.Condition) (string, error) {
	...
	if cond.Op == ast.OpNOT && len(cond.Children) != 1 {
...
func (b *sqlBuilder) comparison(c *config.Config, cond *ast.Condition) (string, error) {
	field, err := b.ident(sqlPhysicalName(c, cond.Field, b.dialect.backend)) // logical -> physical column
	if err != nil {
		return "", err
	}
	vc := config.ValueCaseFor(c, cond.Field) // "" unless the config asked for a case
	dt := valuetype.CondScalarType(c, cond)  // the declared type every literal is read as
	...
	ci := dt == config.FieldString && config.CaseInsensitiveFor(c, cond.Field)
	...
	case ast.OpRegex:
		...
		return b.scalar(field, b.dialect.regexOp, cond.Value, config.FieldString, config.CaseAsIs, false)
	case ast.OpBetween:
		return b.between(field, cond.Value, vc)
	case ast.OpIn:
		return b.inList(field, "IN", ast.OpIn, cond.Value, vc, ci)
	case ast.OpNotIn:
		return b.inList(field, "NOT IN", ast.OpNotIn, cond.Value, vc, ci)
	...
	case ast.OpContainsAny:
		return b.arrayOp(field, ast.OpContainsAny, b.dialect.arrayOverlap, cond.Value, vc)
	case ast.OpContainsAll:
		return b.arrayOp(field, ast.OpContainsAll, b.dialect.arrayContains, cond.Value, vc)
	default:
...
func (b *sqlBuilder) scalar(field, sqlOp string, v *ast.Value, declared config.FieldType, vc config.ValueCase, ci bool) (string, error) {
...
func sliceValue(backend string, op ast.Operator, v *ast.Value, want int) ([]any, error) {
	if v == nil {
...
func (b *sqlBuilder) between(field string, v *ast.Value, vc config.ValueCase) (string, error) {
	elems, err := sliceValue(b.dialect.backend, ast.OpBetween, v, 2)
	if err != nil {
		return "", err
	}
	elems = config.ApplyCaseElems(vc, elems)
...
func (b *sqlBuilder) inList(field, keyword string, op ast.Operator, v *ast.Value, vc config.ValueCase, ci bool) (string, error) {
...
// on MySQL.
func (b *sqlBuilder) arrayOp(field string, op ast.Operator, render func(string, []string) string, v *ast.Value, vc config.ValueCase) (string, error) {
	elems, err := sliceValue(b.dialect.backend, op, v, 0)
	if err != nil {
		return "", err
	}
...
func (b *sqlBuilder) like(field, pre, post string, v *ast.Value, vc config.ValueCase, ci bool) (string, error) {
	if v == nil {
	...
	pattern := pre + config.ApplyCase(vc, s) + post
...
func (b *sqlBuilder) contains(c *config.Config, field string, cond *ast.Condition, vc config.ValueCase, ci bool) (string, error) {
	if f, ok := c.FieldByName(cond.Field); ok && f.Type == config.FieldArray {
		// The bound literal is one element, so it is read as the item type.
		arg, err := b.value(cond.Value, valuetype.ScalarTypeOf(f), vc)
		if err != nil {
			return "", err
		}
		if b.dialect.bindArrayWhole {
...
func (b *sqlBuilder) value(v *ast.Value, declared config.FieldType, vc config.ValueCase) (any, error) {
	if v == nil {
		return nil, fmt.Errorf("%s: operator requires a value", b.dialect.backend)
	}
	if v.Kind == ast.KindRelativeDate { // no payload to read: unit/amount resolve against the clock
	...
	case config.FieldString, config.FieldEnum:
		...
		return config.ApplyCase(vc, s), nil
