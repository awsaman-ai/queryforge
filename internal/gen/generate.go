// Package gen holds the backend generators — SQL (Postgres and MySQL), Mongo,
// Elasticsearch and OpenSearch — together with the small framework they share:
// the Generator interface, the Registry that maps a backend id to one, and the
// helpers (effective limit/offset, projection, predicate ordering, index
// warnings) that every generator applies identically.
package gen
...
type Generator interface {
	...
	Generate(q *ast.Query, c *config.Config, opts GenOptions) (*Result, error) // AST -> backend query
}
...
func effectiveLimit(q *ast.Query, c *config.Config) int {
	if q.Limit != nil {
		return *q.Limit
	}
...
func effectiveOffset(q *ast.Query) int {
	if q.Offset != nil {
		return *q.Offset
	}
	return 0
...
func projectionFields(q *ast.Query, c *config.Config) ([]string, bool) {
	if len(q.Select) > 0 {
...
func orderedChildren(c *config.Config, children []*ast.Condition) []*ast.Condition {
	out := make([]*ast.Condition, len(children))
	copy(out, children)
...
func condSortKey(c *config.Config, cond *ast.Condition) (indexedRank int, priority int) {
	...
	if cond.Type == ast.CondComparison {
...
// hard failures.
func collectIndexWarnings(q *ast.Query, c *config.Config) []string {
	var warnings []string
	seen := make(map[string]bool) // de-duplicate per field
	var walk func(cond *ast.Condition)
	walk = func(cond *ast.Condition) {
		if cond == nil {
			return
		}
		if cond.Type == ast.CondComparison {
...
func ResolveRelative(now time.Time, unit string, amount int) (time.Time, error) {
	if !datetime.IsRelativeUnit(unit) {
		return time.Time{}, fmt.Errorf("relative date has unknown unit %q (expected one of: %s)",
			unit, strings.Join(datetime.RelativeUnits, ", "))
	}
	if amount > datetime.MaxRelativeAmount || amount < -datetime.MaxRelativeAmount {
		return time.Time{}, fmt.Errorf("relative date amount %d is out of range (max magnitude %d)",
			amount, datetime.MaxRelativeAmount)
	}
	switch unit {
	case "minute":
